...
namespace TraverseAllSystems
{
    ...
    public class TreeNode
    {
        ...
        /// </summary>
        public static string CreateJsonParentNode( string id, string label, string json_kids)
        {
            return string.Format( _json_format_to_store_children_in_parent, id, Options.NodeLabelTag, label, json_kids);
        }
        ...
        public static string CreateJsonParentNode( string id, string label, string[] json_kids)
        {
            return CreateJsonParentNode(id, label, string.Join(",", json_kids));
        }
        ...
        /// </summary>
        public void DumpToJsonBottomUp( List<string> json_collector, string parent_id)
        {
            Element e = GetElementById(m_Id);
            string id = GetId(e);

            string json = string.Format( _json_format_to_store_parent_in_child, id, Options.NodeLabelTag, GetName(e), parent_id);
    ...
    public class TraversalTree
    {
        ...
        public TraversalTree(MEPSystem system)
        {
            ...
            m_isPipingSystem = system is PipingSystem;
        ...
        private TreeNode GetStartingElementNode()
        {
            ...
            if (equipment == null)
            {
                ...
                if (elements.Size == 0 && m_isPipingSystem)
                {
                    elements = ((PipingSystem)m_system).PipingNetwork;
                }
                foreach (var element in elements)
                {
                    if (element is FamilyInstance familyInstance)
                    {
                        if (familyInstance.MEPModel is MechanicalEquipment)
                        {
        ...
        private void AppendChildren(TreeNode elementNode)
        {
            ...
            foreach (Connector connector in connectors)
            {
                ...
                {
                    // If the connector is connected to the input connector, they will have opposite flow directions. Then skip it.
            ...
            {
                if (t1.Id.IntegerValue > t2.Id.IntegerValue)
                {
                    return 1;
                }
                else if (t1.Id.IntegerValue < t2.Id.IntegerValue)
                {
                    return -1;
                }
                else
        ...
        public void DumpIntoXML(string fileName)
        {
            ...
            string mepSystemType = m_system is MechanicalSystem ? "MechanicalSystem" : "PipingSystem";
        ...
        private void WriteBasicInfo(XmlWriter writer)
        {
            ...
            writer.WriteValue(elements.Size);
            writer.WriteEndElement();
